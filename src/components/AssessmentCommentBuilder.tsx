  ...
  return (
    <div style={{ minHeight: '100vh', backgroundColor: '#f9fafb' }}>
      <header style={{ backgroundColor: 'white', boxShadow: '0 1px 3px 0 rgba(0,0,0,0.1)', padding: '32px 24px', textAlign: 'center' }}>
        <h1 style={{ fontSize: '28px', fontWeight: '600', color: '#111827', margin: 0 }}>{existingComment ? 'Edit' : 'Create'} Assessment Comment</h1>
      </header>
      <main style={{ maxWidth: '900px', margin: '0 auto', padding: '32px 24px' }}>
        ...
        <div style={{ backgroundColor: 'white', padding: '32px', borderRadius: '12px', boxShadow: '0 1px 3px 0 rgba(0,0,0,0.1)', marginBottom: '24px' }}>
          <div style={{ marginBottom: '32px' }}>
            <label style={{ display: 'block', fontSize: '18px', fontWeight: '600', color: '#111827', marginBottom: '8px' }}>Assessment Comment Name</label>
            <input type="text" placeholder="e.g. End of Unit Test, Class Assessment, Practical Exam..." value={commentName} onChange={e => setCommentName(e.target.value)} style={{ width: '100%', padding: '12px 16px', border: '2px solid #e5e7eb', borderRadius: '8px', fontSize: '16px', boxSizing: 'border-box' }} />
          </div>

          <div style={{ marginBottom: '32px' }}>
            <label style={{ display: 'block', fontSize: '18px', fontWeight: '600', color: '#111827', marginBottom: '8px' }}>Score Entry Type</label>
            <div style={{ display: 'flex', gap: '16px', marginBottom: '16px' }}>
              <label style={{ display: 'flex', alignItems: 'center', gap: '8px', cursor: 'pointer' }}>
                <input type="radio" value="outOf" checked={scoreType === 'outOf'} onChange={e => setScoreType(e.target.value as 'outOf' | 'percentage')} style={{ transform: 'scale(1.1)' }} />
                <span>Score out of (e.g. 15 out of 20)</span>
              </label>
              <label style={{ display: 'flex', alignItems: 'center', gap: '8px', cursor: 'pointer' }}>
                <input type="radio" value="percentage" checked={scoreType === 'percentage'} onChange={e => setScoreType(e.target.value as 'outOf' | 'percentage')} style={{ transform: 'scale(1.1)' }} />
                <span>Percentage (e.g. 75%)</span>
              </label>
            </div>
            {scoreType === 'outOf' && (
              <div>
                <label style={{ display: 'block', fontSize: '14px', fontWeight: '500', color: '#374151', marginBottom: '8px' }}>Default maximum score (teachers can change this when writing reports):</label>
                <input type="number" placeholder="e.g. 20, 100" value={maxScore} onChange={e => setMaxScore(e.target.value)} style={{ width: '100px', padding: '8px 12px', border: '2px solid #e5e7eb', borderRadius: '6px', fontSize: '14px' }} />
              </div>
            )}
          </div>
          ...
          <div style={{ display: 'flex', gap: '16px', justifyContent: 'flex-end' }}>
            ...
            <button onClick={handleSave} style={{ backgroundColor: '#10b981', color: 'white', padding: '16px 32px', border: 'none', borderRadius: '8px', fontSize: '16px', fontWeight: '500', cursor: 'pointer' }}>Save Assessment Comment</button>
          </div>
        </div>
      </main>
    </div>
