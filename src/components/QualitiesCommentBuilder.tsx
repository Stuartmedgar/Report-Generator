...
function QualitiesCommentBuilder({ onSave, onCancel, existingComment }: QualitiesCommentBuilderProps) {
  ...
  const handleSave = () => {
    if (!commentName.trim()) { alert('Please enter a name for this qualities comment'); return; }
    const hasEmptyHeadings = headings.some(heading => !comments[heading] || comments[heading].every(c => !c.trim()));
    if (hasEmptyHeadings) { alert('Please add at least one quality for each area, or remove empty areas'); return; }
  ...
  return (
    <div style={{ minHeight: '100vh', backgroundColor: '#f9fafb' }}>
      <header style={{ backgroundColor: 'white', boxShadow: '0 1px 3px 0 rgba(0,0,0,0.1)', padding: '32px 24px', textAlign: 'center' }}>
        <h1 style={{ fontSize: '28px', fontWeight: '600', color: '#111827', margin: 0 }}>{existingComment ? 'Edit' : 'Create'} Qualities Comment</h1>
      </header>
      <main style={{ maxWidth: '900px', margin: '0 auto', padding: '32px 24px' }}>
        ...
        <div style={{ backgroundColor: 'white', padding: '32px', borderRadius: '12px', boxShadow: '0 1px 3px 0 rgba(0,0,0,0.1)', marginBottom: '24px' }}>
          <div style={{ marginBottom: '32px' }}>
            <label style={{ display: 'block', fontSize: '18px', fontWeight: '600', color: '#111827', marginBottom: '8px' }}>Qualities Comment Name</label>
            <input type="text" placeholder="e.g. Grade 5 Character Qualities, Personal Strengths, Social Skills..." value={commentName} onChange={e => setCommentName(e.target.value)} style={{ width: '100%', padding: '12px 16px', border: '2px solid #e5e7eb', borderRadius: '8px', fontSize: '16px', boxSizing: 'border-box' }} />
          </div>

          {headings.map((heading, hIdx) => (
            <div key={hIdx} style={{ border: `2px solid ${color}`, borderRadius: '12px', padding: '24px', marginBottom: '24px' }}>
              <div style={{ marginBottom: '16px' }}>
                <input type="text" value={heading} onChange={e => updateHeading(hIdx, e.target.value)} style={{ fontSize: '18px', fontWeight: '600', color, border: 'none', backgroundColor: 'transparent', padding: '4px', borderBottom: `2px dashed ${color}`, marginBottom: '8px', width: '100%' }} placeholder="Quality Area Name" />
                {headings.length > 1 && <button onClick={() => removeHeading(hIdx)} style={{ backgroundColor: '#ef4444', color: 'white', border: 'none', borderRadius: '6px', padding: '8px 16px', fontSize: '14px', cursor: 'pointer' }}>Remove Quality Area</button>}
              </div>
              ...
              {showBatchInput === heading && (
                <div style={{ backgroundColor: '#f0f9ff', border: '2px solid #3b82f6', borderRadius: '8px', padding: '16px', marginBottom: '16px' }}>
                  ...
                  <textarea placeholder={separator === 'double-line' ? `[Name] demonstrates exceptional leadership qualities and supports their peers.\n\n[Name] shows great kindness and empathy towards others.` : `[Name] demonstrates exceptional leadership qualities\n[Name] shows great kindness and empathy`} value={batchText} onChange={e => setBatchText(e.target.value)} style={{ width: '100%', height: '200px', padding: '12px', border: '2px solid #bfdbfe', borderRadius: '6px', fontSize: '14px', resize: 'vertical', fontFamily: 'inherit', boxSizing: 'border-box', marginBottom: '12px' }} />
                  <div style={{ display: 'flex', gap: '8px', justifyContent: 'flex-end' }}>
                    ...
                    <button onClick={() => handleBatchPaste(heading)} disabled={!batchText.trim()} style={{ backgroundColor: batchText.trim() ? '#3b82f6' : '#d1d5db', color: 'white', border: 'none', borderRadius: '6px', padding: '8px 16px', fontSize: '14px', cursor: batchText.trim() ? 'pointer' : 'not-allowed' }}>Add Qualities</button>
                  </div>
                </div>
              )}
          ...
          <div style={{ marginBottom: '32px', textAlign: 'center' }}>
            <button onClick={addHeading} style={{ backgroundColor: '#a855f7', color: 'white', border: 'none', borderRadius: '8px', padding: '12px 24px', fontSize: '16px', fontWeight: '500', cursor: 'pointer' }}>+ Add Another Quality Area</button>
          </div>
          ...
          <div style={{ display: 'flex', gap: '16px', justifyContent: 'flex-end' }}>
            ...
            <button onClick={handleSave} style={{ backgroundColor: color, color: 'white', padding: '16px 32px', border: 'none', borderRadius: '8px', fontSize: '16px', fontWeight: '500', cursor: 'pointer' }}>Save Qualities Comment</button>
          </div>
        </div>
      </main>
    </div>
